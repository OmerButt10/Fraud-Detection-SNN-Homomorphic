...
class FraudSNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Initialize membrane potentials and keep them persistent across time steps
        mem1 = torch.zeros(batch_size, self.fc1.out_features, device=x.device)
        mem2 = torch.zeros(batch_size, 1, device=x.device)

        spk_sum = torch.zeros(batch_size, 1, device=x.device)

        for t in range(self.time_steps):  # Loop through time steps
            cur1 = self.fc1(x[:, t, :])  # Compute input current
            spk1, mem1 = self.lif1(cur1, mem1)  # Persistent membrane update

            cur2 = self.fc2(spk1)  
            spk2, mem2 = self.lif2(cur2, mem2)  # Persistent membrane update

            spk_sum += spk2  # Accumulate spikes

        spk_mean = spk_sum / self.time_steps  # Average firing rate
        out = self.sigmoid(spk_mean)  # Sigmoid activation
