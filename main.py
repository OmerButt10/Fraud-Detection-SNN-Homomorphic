...
from snn_hfe import (
    ...
    compare_encrypted_inputs,
    compare_encrypted_outputs
    )


# -------------------------
# Create the directory for encryption comparison
# -------------------------
os.makedirs("encryption_comparison", exist_ok=True)
...
ts_input_buffer = tenseal_enc_input.serialize()
ts_encrypted_input_str = base64.b64encode(ts_input_buffer).decode('utf-8')
# Encrypt the input
...
# Save to a text file
input_text = f"""
CONCRETE Encrypted Input Length:\t {len(encrypted_input_str)}
TENSEAL Encrypted Input Length:\t {len(ts_encrypted_input_str)}
CONCRETE Encrypted Input:\t {encrypted_input_str}
TENSEAL Encrypted Input:\t {ts_encrypted_input_str}
"""
with open("encryption_comparison/encryption_input.txt", "w") as file:
    file.write(input_text)

# compare_encrypted_inputs(tenseal_enc_input, q_input_enc, input_buffer, encrypted_input_str)
...
ts_output_buffer = tenseal_enc_output.serialize()
ts_encrypted_output_str = base64.b64encode(ts_output_buffer).decode('utf-8')
# Save to a text file
output_text = f"""
CONCRETE Encrypted Input Length:\t {len(encrypted_output_str)}
TENSEAL Encrypted Input Length:\t {len(ts_encrypted_output_str)}
CONCRETE Encrypted Input:\t {encrypted_output_str}
TENSEAL Encrypted Input:\t {ts_encrypted_output_str}
"""
with open("encryption_comparison/encryption_output.txt", "w") as file:
    file.write(output_text)


# tenseal_enc_output = encrypt_with_tenseal(q_y, context)

# compare_encrypted_outputs(tenseal_enc_output, q_y_enc, output_buffer, encrypted_output_str)
...
y_proba_fhe, execution_time = get_normal_output(scripted_model, torch_input)
